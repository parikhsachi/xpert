...
def recency_score(year: int, decay_factor: float = 0.5, half_life: int = 10) -> float:
    current_year = datetime.now().year
    age = max(0, current_year - year)
    return decay_factor ** (age / half_life)

# Give paper a score
def combined_paper_score(index: int, total: int, citations: int, year: int, weights=(0.5, 0.3, 0.2)) -> float:
